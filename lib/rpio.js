 ...
 */
function detect_pinmap(procPath)
{
	var model;

	try {
		model = fs.readFileSync(procPath || '/proc/device-tree/model', 'ascii');
...
rpio.prototype.init = function(opts)
{
	...
	if (rpio_options.mock) {
		...
		if (!detect_pinmap(rpio_options.procPath)) {
...
rpio.prototype.i2cReadRegisterRestart = function(reg, buf, len)
{
	...
	return bindcall3(binding.i2c_read_reg_rs, reg, buf, len);
}
